...
[range, angleV, angleH, timestamp] = ...
    read_scandata("calibr_data\batchScanned20200626210606.txt");
...
% wall
phi1_1 = 1421; phi1_2 = 2180; theta1_1 = 440;  theta1_2 = 530;
phi2_1 = 2901; phi2_2 = 3200; theta2_1 = 161; theta2_2 = 260;
phi3_1 = 21; phi3_2 = 460; theta3_1 = 161; theta3_2 = 260;
...
phi6_1 = 21; phi6_2 = 3220; theta6_1 = 318; theta6_2 = 379;
...
if cropping
    ...
    ps5 = range2pointsPrecise(range_crop6, angleV_crop6, angleH_crop6, R,Dphi,Dpsi,Dtheta);
...
% iter = 60;
% Rstep = 0.01;
% [R, err2] = calibrate_R(d1,theta1,phi1, d2,theta2,phi2,...
...
% [Dphi, Dpsi, err3] = calibrate_DphiDpsi(d1,theta1,phi1,d2,theta2,phi2, ...
%     R, Dphi, Dpsi, Dtheta, iter, lr, msgOn) ;
% Dphi = 1.45;
% [R, Dpsi, err3] = calibrate_R_Dpsi(d1,theta1,phi1,d2,theta2,phi2, ...
%     R, Dphi, Dpsi, Dtheta, iter, lr, msgOn) ;

%% display raw pointcloud 
ps0_cabr = range2pointsPrecise_ts(range, angleV, angleH, R,Dphi,Dpsi,Dtheta);
...
% show the patches
ps1_cabr = range2pointsPrecise_ts(range_crop1, angleV_crop1, angleH_crop1, R,Dphi,Dpsi,Dtheta);
ps2_cabr = range2pointsPrecise_ts(range_crop2, angleV_crop2, angleH_crop2, R,Dphi,Dpsi,Dtheta);
ps3_cabr = range2pointsPrecise_ts(range_crop3, angleV_crop3, angleH_crop3, R,Dphi,Dpsi,Dtheta);
ps_cabr = [ps1_cabr;ps2_cabr;ps3_cabr];
figure(13); scatter3(ps_cabr(:,1),ps_cabr(:,2),ps_cabr(:,3),1,'.'); 
xlabel('x'); ylabel('y'); zlabel('z'); 
az = 90; el = 0; view(az,el)

ps_ceil_cabr = range2pointsPrecise_ts(range_crop6, angleV_crop6, angleH_crop6, R,Dphi,Dpsi,Dtheta);
