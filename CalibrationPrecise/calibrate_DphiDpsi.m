function [Dphi, Dpsi, err] = calibrate_DphiDpsi(d1,theta1,phi1,d2,theta2,phi2, ...
    R, Dphi, Dpsi, Dtheta, iter, lr, msgOn) 
% Function: calibrate long-arm LIDAR parameters.
% Method: use z-x-y nautical angles 
% Method: simultaneously calibrate Dphi, Dpsi on a wall by gradient descent.
% Input:
...
% Output:
%     Dphi, Dpsi - calibrated parameters and error
...
for ii = 1:iter
    ...
    for i = 1:H1 % for each scanning plane
        ...
        dx1_alpha((1:V1)+V1*(i-1),1:3) = dx_dphi;
        dx1_alpha((1:V1)+V1*(i-1),4:6) = dx_dpsi;
    ...
    for i = 1:H2 % for each scanning plane
        ...
        dx2_alpha((1:V2)+V2*(i-1),1:3) = dx_dphi;
        dx2_alpha((1:V2)+V2*(i-1),4:6) = dx_dpsi;
    ...
    if msgOn
        figure(102); plot(ii, log(err(ii)), 'r*');
        title(' Dphi, Dpsi - log error')
        xlim([0,iter]); hold on
    end
    
    % gradient of error
    dX = [dx1_alpha;dx2_alpha];
    derr_Dphi = sum(dX(:,1:3)*c.*err_plane)/K;
    derr_Dpsi = sum(dX(:,4:6)*c.*err_plane)/K;
    mag = sqrt(derr_Dpsi*derr_Dpsi+derr_Dphi*derr_Dphi);
